...
@Client.on_message(filters.private & ~filters.forwarded & filters.command(["استخراج جلسه", "• استخراج جلسه •", "• ََِِ𝗲ًًٍٍ𝗫ٖٔ𝗧ًًٍٍ𝗥ّّ𝗮ََِِ𝗰ٖٔ𝗧 ّّ𝗮 ٍّّ𝘀ََِِ𝗲ٍّّ𝘀ٍّّ𝘀ٍٍَ𝗶ُُ𝗼ٍٍّّ𝗡 •"], ""))
...
async def generate_session(bot: Client, msg: Message, telethon=False, is_bot: bool = False):
    ...
    user_id = msg.chat.id
    api_id_msg = await bot.ask(user_id, "ارسل api_id\nاذا لم تكن معك قم بكتابه تخطي", filters=filters.text)
    ...
    if api_id_msg.text == "تخطي":
        ...
        try:
            ...
            return
        api_hash_msg = await bot.ask(user_id, "» تم حفظ api_id \nارسل api_hash", filters=filters.text)
        if await cancelled(api_hash_msg):
            return
        api_hash = api_hash_msg.text
    if not is_bot:
        t = "» ارسل الينا رقم الهاتف لكي نرسل لك كود. \nمثال علي رقم الهاتف : `+910000000000`'"
    else:
    ...
    if not is_bot:
        await msg.reply("» جار ارسال الكود...")
    else:
    ...
    if not is_bot:
        ...
        try:
            ...
            try:
                two_step_msg = await bot.ask(user_id, "» الحساب محمي بكلمه سر قم بكتابه كلمه السر", filters=filters.text, timeout=300)
            except TimeoutError:
    ...
    text = f"**تم استخراج جلستك بنجاح** \n\n`{string_session}`\n\n**استخرجت بواسطه ʙʏ :** @EFFB0T"
    try:
        if not is_bot:
    ...
    await bot.send_message(msg.chat.id, f"» تم استخراج جلستك بنجاح.\n\n`{string_session}`\nايضا تم ارسالها في المحفوظات الخاصه بالحساب\n\n**SOURC ROCK:- @EFFB0T**")
