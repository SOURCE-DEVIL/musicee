...
from ESLAM.info import (remove_active, is_served_call, joinch)
from ESLAM.Data import (get_call, get_dev, get_group, get_channel, get_dev_user)
from ESLAM.info import (add, db, download, gen_thumb)
...
async def admin_risghts(client: Client, CallbackQuery):
  try:
    ...
    if command == "stop":
        ...
        await CallbackQuery.answer("تم انهاء التشغيل بنجاح 🧚‍♀️", show_alert=True)
        await CallbackQuery.message.reply_text(f"{CallbackQuery.from_user.mention} **تم انهاء التشغيل بواسطه**")
  except:
     pass

@Client.on_message(filters.command(["/stop", "/end", "/skip", "/resume", "/pause", "/loop", "ايقاف مؤقت", "استكمال", "تخطي", "انهاء", "اسكت", "ايقاف", "تكرار", "كررها", "وقف", "كمل"], "") & ~filters.private)
    ...
    if command == "/pause" or command == "ايقاف مؤقت" or command == "وقف":
       ...
       if videoid:
         if userx.photo:
            ...
            ahmed = await client.get_chat("LLL3P")
